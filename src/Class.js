...
class CrudGist {
   ...
   */
  constructor({ token, username, gistId, filename }) {
    this.token = token;
  ...
  async paged() {
    try {
      return this.Github.paged(`/users/${this.username}/gists`).then(
        (res) => res.pages[0].body
      );
    } catch (error) {
  ...
  async get() {
    try {
      return await this.Github.get(`/gists/${this.gistId}`).then((response) =>
        JSON.parse(response.body.files[this.fileName].content)
      );
    } catch (error) {
      console.log(error);
      return {};
